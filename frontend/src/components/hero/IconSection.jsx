...
function IconSection() {
    ...
    return (
            <Container>
                <Stack  sx={{bgcolor:theme.palette.mode =="dark" ? "#000":"#fff",mt:3,ml:"-20px",mr:"-20px"}} 
                    divider={
...
const MyBox = ({ icon, title, subTitle }) => {
    const theme = useTheme();
    return (
        <Box sx={{display:"flex",width:250,flexGrow:1,gap:3,justifyContent:useMediaQuery("(min-width:600px)") ? "center": "left",py:1.7}}>
            {icon}
